...
// as it is loaded.
namespace { 
  class ModuloStream : public pp::Module {
 public:
  ModuloStream() : pp::Module() {}
  virtual ~ModuloStream() {}
...
class StreamVideo : public pp::Instance,public pp::Graphics3DClient {
 ...
 private:
  ...
  pp::CompletionCallbackFactory<StreamVideo> callback_factory_;
...
void StreamVideo::DidChangeView(
...
void StreamVideo::HandleMessage(const pp::Var& var_message) {
  ...
  if (command == "init") {
    ...
    video_track_ = pp::MediaStreamVideoTrack(resource_track);//Instanciando
    ConfigureTrack();
...
void StreamVideo::Render() {
  ...
  pp::CompletionCallback cb = callback_factory_.NewCallback(
      &StreamVideo::PaintFinished);
  context_->SwapBuffers(cb);
}

void StreamVideo::PaintFinished(int32_t result) {
  is_painting_ = false;
  if (needs_paint_)
    Render();
}

GLuint StreamVideo::CreateTexture(
...
void StreamVideo::CreateGLObjects() {
...
void StreamVideo::CreateTextures() {
...
void StreamVideo::ConfigureTrack() {
  ...
  video_track_.Configure(attrib_list, callback_factory_.NewCallback(
        &StreamVideo::OnConfigure)); //Configurando
}

void StreamVideo::OnConfigure(int32_t result) {
  video_track_.GetFrame(callback_factory_.NewCallbackWithOutput(
      &StreamVideo::OnGetFrame));//Pegando o frame
}

void StreamVideo::OnGetFrame(
  ...
  if (!is_bgra_) {//Desenha em um dos lados 
    ...
    glActiveTexture(GL_TEXTURE3);//opengl
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height,
  ...
  if (need_config_) {//Se precisar configurar
    ConfigureTrack();
    need_config_ = false;
  } else {//Se não pege o proximo frame denovo
    video_track_.GetFrame(callback_factory_.NewCallbackWithOutput(
        &StreamVideo::OnGetFrame));
  }
}

pp::Instance* ModuloStream::CreateInstance(PP_Instance instance) {
  return new StreamVideo(instance, this);
}
